...
```{r}
fraud <- read.csv("D:/Downloads/base examen écrit.csv")
```
...
```{r}
fraud %>% count(is_fraud) %>% ggplot(aes(x="",n,fill=is_fraud)) + geom_bar(stat = "identity",width = 0.5) + coord_polar("y") +
  ggtitle("Percentage of every class") + scale_fill_manual(values=c("#9933FF","#33FFFF"))
```
